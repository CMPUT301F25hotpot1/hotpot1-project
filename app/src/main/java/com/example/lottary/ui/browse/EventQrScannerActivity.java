 ...
 *
 * Purpose:
 * - Open camera, scan a QR code generated by QrScanActivity (or any generic
 *   QR app) on another device.
 * - The QR text can be:
 *      1) "lottary://event/<eventId>"  (deep-link format, recommended)
 *      2) "<eventId>"                  (legacy plain id)
 * - We normalize this into an event id, then launch EventDetailsActivity
 *   with EXTRA_EVENT_ID = that id.
 */
public class EventQrScannerActivity extends AppCompatActivity {
    ...
    /**
     * Configure CameraX + ML Kit barcode scanner.
     */
    ...
    /**
     * Analyze a single frame and try to decode barcodes.
     */
                    ...
                    .addOnSuccessListener(barcodes -> {
                        ...
                        for (Barcode b : barcodes) {
                            ...
                            if (value != null && !value.isEmpty()) {
                                ...
                                // 1) "lottary://event/<id>" (deep-link)
                                // 2) "<id>" (plain event id)
                                String eventId = value;
                                String prefix = "lottary://event/";
                                if (value.startsWith(prefix)) {
                                    eventId = value.substring(prefix.length());
                                }
